...
try {
    ...
    # TLS 1.2 pour les téléchargements (requis par PowerShell 5.1)
    ...
    if (-not $claudeApp -and -not $claudeCli) {
        ...
        try {
            ...
            Warn "Installeur officiel indisponible ($($_.Exception.Message))"
            if (Get-Command npm -ErrorAction SilentlyContinue) {
                Warn "Tentative via npm..."
        ...
        $claudeCli = Get-Command claude -ErrorAction SilentlyContinue
        if (-not $claudeCli) {
            $localBin = Join-Path $TargetUserProfile '.local\bin\claude.exe'
            if (Test-Path $localBin) { $claudeCli = Get-Command $localBin }
        }
        if ($claudeCli) {
            Ok "Claude Code installé : $($claudeCli.Source)"
        } else {
            Warn "Installation de Claude échouée - installez-le depuis https://claude.ai/download. Le skill est prêt et sera détecté au premier lancement."
        }
    }
...
'@
    ...
    if ($claudeApp) {
        ...
        $cliExe  = $claudeCli.Source
        $cliArgs = if ($hasProject) { "-NoExit -Command `"Set-Location '$ProjectPath'; & '$cliExe'`"" }
                   else             { "-NoExit -Command `"& '$cliExe'`"" }
